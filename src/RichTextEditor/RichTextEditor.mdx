...
RichTextEditor can be configured to be used in display mode only which is useful when displaying rich text when you have to do something extra with it (e.g character truncation).
Modify `displayMode` to `true`. This will automatically put the editor in not editable mode as well.
